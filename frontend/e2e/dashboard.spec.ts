...
test('dashboard guides a user from empty state to book, capture, and project focus', async ({ page, request }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Complete the first valuable loop' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Start first loop' }).first()).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Start with one book, one capture, and one source check.' })).toBeVisible()
  const firstDayPanel = page.getByRole('region', { name: 'Start with one book, one capture, and one source check.' })
  await expect(firstDayPanel.getByRole('button', { name: 'Add Book' })).toBeVisible()
  await expect(firstDayPanel.getByRole('button', { name: 'Practice in Demo' })).toBeVisible()
...
test('dashboard mode switcher changes landing focus and persists after reload', async ({ page }) => {
  const runId = uniqueRunId()
  ...
  await expect(page.getByRole('heading', { name: 'Apply Knowledge', exact: true })).toBeVisible()
